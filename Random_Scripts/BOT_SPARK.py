...
# BOT'S ACCESS TOKEN on CISCO SPARK
bearer = ""
headers = {
...
def ngrok():
    if platform.system() == "Windows":
        ...
        print("Checking if ngrok is present in current folder...")
        check_ngrok = subprocess.Popen(["ls -al ./ | grep ngrok"], shell=True, stdout=subprocess.PIPE,
        ...
        if "x" in check_ngrok:
            ...
            set_ngrok = subprocess.Popen(
                ["wget https://bin.equinox.io/c/4VmDzA7iaHb/ngrok-stable-darwin-amd64.zip" \
                 "&& unzip ngrok-stable-darwin-amd64.zip"], shell=True, stdout=subprocess.PIPE,
                stderr=subprocess.STDOUT)
            while set_ngrok.poll() is None:
                continue
            check_ngrok = subprocess.Popen(["ls -al ./ | grep ngrok"], shell=True, stdout=subprocess.PIPE,
